...
void pow_a(int *a, int *b, int n, int m, int comm_sz /* 总进程数 */) {
  // DONE: 对这个进程拥有的数据计算 b[i] = a[i]^m
  int local_n = n / comm_sz;
  for (int i = 0; i < local_n; i++) {
    int x = 1;
    for (int j = 0; j < m; j++) x *= a[i];
    b[i] = x;
  }
}
...
int main(int argc, char **argv) {
  ...
  if (my_rank == 0) {
    ...
    for (int i = 0; i < n; i++) root_a[i] = rand() % 1024;
  }
  ...
  // 进程 i 获取进程 0 中位于 [my_rank * (n / comm_sz), (my_rank + 1) * (n /
  // comm_sz)) 位置的数据
  MPI_Scatter(root_a, n / comm_sz,
              MPI_INT,                  // send_buf_p, send_count, send_type
  ...
  // 进程 0 收集各进程的运算结果
  MPI_Gather(b, n / comm_sz, MPI_INT,       // send_buf_p, send_count, send_type
